...
MyWindow::MyWindow(QApplication *parent) : QMainWindow(), parent(parent)
{
  ...
  connect(viewer, SIGNAL(emitTopology(int, int, int)), this, SLOT(showTopology(int,int,int)));
...
void MyWindow::open()
{
  QString fileName =
    QFileDialog::getOpenFileName(this, tr("Open File"), ".",
                                 tr("PLY Mesh (*.ply);;STL Mesh (*.stl);;OBJ Mesh (*.obj);;All files (*.*)"));
  if(!fileName.isEmpty()) 
...
void MyWindow::showTopology(int vertices, int faces, int edges)
{
    QLabel* label = new QLabel(this);
    label->setFrameStyle(QFrame::Panel | QFrame::Sunken);
    label->setAlignment(Qt::AlignTop | Qt::AlignLeft);
    label->setGeometry(1, 28, 300, 40);
    label->setStyleSheet("QLabel { background-color : gray; color : white; font: bold 20px; }");
    label->setText(tr("V: ") + QString::number(vertices) + tr("\t") +
                   tr("F: ") + QString::number(faces) + tr("\t") +
                   tr("E: ") + QString::number(edges));
    label->show();
}
